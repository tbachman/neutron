...
class N1KVInterfaceDriver(LinuxInterfaceDriver):
    """Driver for creating an internal interface on Cisco Nexus1000V switch."""

    DEV_NAME_PREFIX = n_const.TAP_DEVICE_PREFIX

    def __init__(self, conf):
        super(N1KVInterfaceDriver, self).__init__(conf)
        self.DEV_NAME_PREFIX = 'ns-'
    ...
    def _n1kv_add_port(self, device_name, port_id, mac_address):
        cmd = ['vemctl',
               'add-port', '--name', device_name,
               '--type', 'virt',
               '--id', port_id,
    ...
    def plug(self, network_id, port_id, device_name, mac_address,
        ...
        if not ip_lib.device_exists(device_name,
            ...
            root_dev, ns_dev = ip.add_veth(tap_name,
                                           device_name,
                                           namespace2=namespace)
            self._n1kv_add_port(tap_name, port_id, mac_address)
            ns_dev.link.set_address(mac_address)
    ...
    def unplug(self, device_name, bridge=None, namespace=None, prefix=None):
        ...
        tap_name = self._get_tap_name(device_name, prefix)
        try:
            cmd = ['vemctl', 'del-port', '--name', tap_name]
            utils.execute(cmd, self.root_helper)
            device = ip_lib.IPDevice(device_name,
                                     self.root_helper,
                                     namespace)
