...
from neutron.plugins.ml2.drivers.cisco.n1kv import exceptions as c_exc
...
def _create_test_policy_profile_if_not_there(session,
    ...
    try:
        ...
        _profile = n1kv_db.add_policy_profile(profile['id'], profile['name'])
    return _profile
...
class PolicyProfileTests(testlib_api.SqlTestCase):
    ...
    def test_create_policy_profile(self):
        _db_profile = n1kv_db.add_policy_profile(TEST_POLICY_PROFILE['id'],
    ...
    def test_delete_policy_profile(self):
        ...
        n1kv_db.remove_policy_profile(profile.id)
        try:
    ...
    def test_get_policy_profiles(self):
        profile = _create_test_policy_profile_if_not_there(self.session)
        got_profile = n1kv_db.get_policy_profiles().one()
        self.assertEqual(profile.id, got_profile.id)
        self.assertEqual(profile.name, got_profile.name)
