...
class TestCiscoApicMechDriver(base.BaseTestCase,
    ...
    def test_update_port_postcommit(self):
        ...
        mgr.ensure_tenant_created_on_apic.assert_called_once_with(
            mocked.APIC_TENANT)
        mgr.ensure_path_created_for_port.assert_has_calls([
            mock.call(mocked.APIC_TENANT, mocked.APIC_NETWORK, HOST_ID1,
                      ENCAP, mocked.APIC_NETWORK + '-name'),
            mock.call(mocked.APIC_TENANT, mocked.APIC_NETWORK, None,
                      ENCAP, mocked.APIC_NETWORK + '-name')])
