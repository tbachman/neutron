...
class TestCiscoNexusDevice(base.BaseTestCase):
    ...
    def _verify_results(self, driver_result):
        ...
        for idx in xrange(0, len(driver_result)):
            self.assertNotEqual(self.mock_ncclient.connect.
                                return_value.edit_config.
                                mock_calls[idx][2]['config'],
    ...
    def test_create_delete_duplicate_ports(self):
        """Tests creation and deletion of two new virtual Ports."""
        duplicate_add_port_drvr_result = ['\<vlan\-name\>q\-267\<\/vlan\-name',
                                          '\<vstate\>active\<\/vstate>',
                                          '\<no\>\s+\<shutdown\/\>\s+\<\/no\>',
                                          '\<interface\>1\/10\<\/interface\>'
                                          '\s+[\x20-\x7e]+\s+\<switchport\>\s+'
                                          '\<trunk\>\s+\<allowed\>\s+\<vlan\>'
                                          '\s+\<vlan_id\>267', ]
        duplicate_delete_port_drvr_result = ['\<interface\>1\/10\<\/interface'
                                             '\>\s+[\x20-\x7e\s]+\<switchport'
                                             '\>\s+\<trunk\>\s+\<allowed\>\s+'
                                             '\<vlan\>\s+\<remove\>\s+\<vlan'
                                             '\>267',
                                             '\<no\>\s+\<vlan\>\s+'
        ...
        self._verify_results(duplicate_add_port_drvr_result)
        ...
        self._verify_results(duplicate_delete_port_drvr_result)
...
class TestCiscoNexusReplay(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        def new_nexus_init(mech_instance):
            ...
            mech_instance.driver.credentials = (
                TestCiscoNexusReplay.test_credentials)
