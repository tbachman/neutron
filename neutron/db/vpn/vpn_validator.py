...
class VpnReferenceValidator(object):
    ...
    def validate_ipsec_site_connection(self, context, ipsec_sitecon,
                                       ip_version, prev_conn=None):
        """Reference implementation of validation for IPSec connection.

        As a side effect, this will also set DPD attributes that are not
        provided. For create, these will be the default values for the
        reference implementation. For update validation, the connection's
        previous values are used for DPD attributes that are not provided.
        """
        if not prev_conn:
