...
class ConfigSyncer(object):
    ...
    def clean_snat(self, conn, router_id_dict, intf_segment_dict, segment_nat_dict, parsed_cfg):
        ...
        for snat_rule in floating_ip_nats:
            ...
            LOG.info("   in_ip: %s, out_ip: %s, router_id: %s, hsrp_num: %s,"
                     " segment_id: %s" %
                     (inner_ip, outer_ip, router_id, hsrp_num, segment_id))
    ...
    def subintf_real_ipv6_check(self,
        ...
        for target_intf in intf_list:
            if target_intf['device_owner'] == target_type:
                ...
                LOG.info("target ip,prefixlen: %s,%s, actual ip,"
                         "prefixlen %s,%s" %
                         (target_ip, target_prefixlen, ipv6_addr, prefixlen))
