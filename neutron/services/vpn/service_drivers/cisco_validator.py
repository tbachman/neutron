...
class CiscoCsrVpnValidator(vpn_validator.VpnReferenceValidator):
    ...
    def validate_ipsec_site_connection(self, context, ipsec_sitecon,
        ...
        to the Cisco CSR. Note, the reference validation is done first, and
        it has a side effect of setting DPD values that are not provided.
        """
        super(CiscoCsrVpnValidator, self).validate_ipsec_site_connection(
            context, ipsec_sitecon, ip_version, prev_conn)
